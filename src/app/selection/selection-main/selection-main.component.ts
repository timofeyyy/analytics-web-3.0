...
@Component({
  ...
  imports: [HttpClientModule, NavigatorComponent, LoaderComponent, NgStyle, SelectListComponent, NgFor, NgClass, SelectListCheckboxComponent, ParameterSortingComponent, PageLabelsComponent, NgIf, ComponentAnalyticsComponent, ReportBuilderComponent, EmptyValuePipe],
...
export class SelectionMainComponent implements OnInit {
      ...
      .pipe(
        ...
        concatMap((typeNames) => {
          ...
          if (!typeNames || !typeNames.length) {
            // сделать редирект на ошибку 404
          }
  ...
  initColumns(): void {
    ...
    if (this.mainAction === AppEnum.ALLREC) {
      rows = (this.storageAll[this.cts.getCurrentComponentType().enComponentType.toLowerCase()])
      storage = this.storageAll
      this.storage = this.storageAll
    }
    else if (this.priority != AppEnum.ALLREC) {
      const entries = Object.entries(this.storageFiltered);
      const first = new Map(entries).keys().next().value
