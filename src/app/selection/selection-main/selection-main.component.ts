...
export class SelectionMainComponent implements OnInit {
    ...
    this.api.getPdfPath()
      .pipe(
        ...
        concatMap((typeNames) => {
          ...
          return this.api.getColumns(componentTypeRes?.EnComponentType)
        }),
        ...
        //   return this.api.getChartColumns()
        // }),
        concatMap((allColumns: { [type: string]: Columns[] }) => {
           this.cts.setColumnsAll(allColumns)
          this.changeSpecificationPath(this.cts.getCurrentComponentType())
          // this.chartColumns = Object.entries(chartColumns).map(column => column)
          return this.api.getComponentsAll(query)
        })
      )
      .subscribe((res: any) => {
        ...
        if (!isNaN(lastDate.getDate()) && !isNaN(fromDate.getDate())) {
          ...
          console.log(storageAllTmp)
          for (const key in storageAllTmp) {
            const all = (storageAllTmp[key] as any[]).filter(item => {
              let dateStr = item['InsertionDate']
              if (dateStr) {
                // dateStr = dateStr.replace('T', ' ').split(' ')[0]
                return dateStr >= this.fromDateStr && dateStr <= this.lastDateStr
              }
              return true
            })
  ...
  initColumns(): void {
    ...
    this.cts.getComponentTypes().forEach((val, index) => {
      if (this.storageAll && this.storageAll[val.EnComponentType]) {
    ...
    this.all = (this.storageAll[this.cts.getCurrentComponentType().EnComponentType])
    this.resultBuffer = Array.from(this.all)
    const colsMap = this.selectedColumnsMap.get(this.cts.getCurrentComponentType().EnComponentType)
    if (colsMap) {
  ...
  onColumnSelected(column: string) {
    ...
    this.selectedColumnsMap.set(this.cts.getCurrentComponentType().EnComponentType, { current: column, cols: JSON.parse(JSON.stringify(this.activeColumns)) })
    this.setSortingParam()
    this.selectPage(0)
  }
  records: any[] = []
  ...
  onColumnsSelected(columns: string[]) {
    ...
    this.selectedColumnsMap.set(this.cts.getCurrentComponentType().EnComponentType, { current: this.activeColumns.current[0], cols: JSON.parse(JSON.stringify(this.activeColumns)) })
    this.setSortingParam()
    this.selectPage(0)
    this.cdr.detectChanges()
  }
