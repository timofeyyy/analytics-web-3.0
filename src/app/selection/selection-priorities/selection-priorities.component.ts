...
@Component({
  ...
  imports: [HttpClientModule, NavigatorComponent, LoaderComponent, NgStyle, SelectListComponent, NgFor, NgClass, SelectListCheckboxComponent, ParameterSortingComponent, PageLabelsComponent, NgIf, ComponentAnalyticsComponent, ReportBuilderComponent, EmptyValuePipe, InputNumericComponent, ChartTemplateComponent],
...
export class SelectionPrioritiesComponent implements OnInit {
  ...
  windowState: "records" | "parameters" | "selection" = "records"
    ...
    this.api.getPdfPath()
      .pipe(
        ...
        concatMap((allColumns: { [type: string]: Columns[] }) => {
          ...
          const columns = allColumns[this.cts.getCurrentComponentType()!.EnComponentType]
      ...
      .subscribe((res: any) => {
        this.sroragePriorities = Object.fromEntries(new Map().set(this.cts.getCurrentComponentType().EnComponentType, res[1]))
        this.storageAll = res[0]
        this.storageFiltered = Object.fromEntries(new Map().set(this.cts.getCurrentComponentType().EnComponentType, res[1]))
        console.log(Object.entries(this.storageFiltered[this.cts.getCurrentComponentType().EnComponentType]))
        this.selectionChartData = Object.entries(this.storageFiltered[this.cts.getCurrentComponentType().EnComponentType]).map((entry) => (entry[1] as []).length) as number[]
        ...
        for (const pr of this.priorities) {
          const storage: any = {}
          const data = this.storageFiltered[first!][pr[0]]
          storage[first!] = data
          this.prioritiesBuffers.push([pr[0], storage])
        }
        // this.cts.getCurrentComponentType().EnComponentType
  ...
  initColumns(): void {
    ...
    const entries = Object.entries(this.storageFiltered);
    const first = new Map(entries).keys().next().value
    this.cts.setCurrentComponentType(this.cts.getComponentTypes().find(val => val.EnComponentType == first)!)
    rows = this.storageFiltered[first!][this.priority]
    storage = this.storageFiltered
    if (rows) {
      const storageTmp = Object.assign({}, this.storageAll)
      storageTmp[this.cts.getCurrentComponentType().EnComponentType] = rows
    ...
    this.cts.getComponentTypes().forEach((val, index) => {
      if (storage && storage[val.EnComponentType]) {
  ...
  onChartChange(data: any, param: string | void): void {
    ...
    data['resultBuffer'] = {}
    data['resultBuffer'][componentType.EnComponentType] = this.storageFiltered[componentType.EnComponentType][actualParam]
