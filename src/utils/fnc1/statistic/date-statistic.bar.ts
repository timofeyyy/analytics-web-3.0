...
export const getDateStatisticBar = (data: any, query: Map<string, string>): Partial<ChartOptions> => {
    ...
    const dataDividedByDates = [
        {
            data: data[EnComponentType!][currentDate1!].sort((a: any, b: any) => {
        ...
        {
            data: data[EnComponentType!][currentDate2!].sort((a: any, b: any) => {
                const diff = (a[param] ?? 0) - (b[param] ?? 0);
                return -diff;
            }),
            name: currentDate2
